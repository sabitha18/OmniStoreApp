...
object MatchWithAddToCartBottomSheetDialog {
    ...
    private var mainActivity: MatchWithActivity? = null
    ...
    fun showProduct(
        activity: Activity,
        viewmodel: MatchWithViewModel,
        bm: Related,
        showBack: Boolean,
        nestedScrollView: NestedScrollView
    ) {
        parentRootView = nestedScrollView
        mainActivity = activity as MatchWithActivity
        matchWithViewModel = viewmodel
...
//        showConfig(bm, activity, globalClass)
    ...
    private fun loadProductDetail(
        ...
        matchWithViewModel.getMatchProductDetails(selectedLanguage, selectedCurrency, bm.id)
        matchWithViewModel.responseMAtchProductDetails.observe(
            mainActivity!!
        ) {
            when (it) {
                is Resource.Success -> {
...
//                    .toFloat()
    ...
    private fun changeConfig(
        ...
        matchWithViewModel.changeConfig(selectedLanguage, configAttr)
        matchWithViewModel.responseChangeConfig.observe(
            mainActivity!!
        ) {
            when (it) {
                is Resource.Success -> {
    ...
    private fun addToCart(
        ...
        matchWithViewModel.addToCart(selectedLanguage, selectedCurrency, reqModel)
        matchWithViewModel.responseAddToCart.observe(
            mainActivity!!
        ) {
            when (it) {
                is Resource.Success -> {
...
//    }
    ...
    fun omniScanItem(skuCode: String) {
        ...
        matchWithViewModel.omniScanItem(
            skuCode,
            storeId!!.toString(),
            priceListId!!.toString()
        )
        matchWithViewModel.responseScanItemOmni.observe(mainActivity!!) {
            when (it) {
                is Resource.Success -> {
                    dismissProgress()
                    btmSheetConfig?.dismiss()
...
//                        model!!.remaining_qty = intRmngQty
                ...
                is Resource.Error -> {
                    ...
                    try {
                        ...
                        exception.printStackTrace()
                        Utils.showSnackbar(parentRootView!!, it.message.toString())
                    }
                }
                is Resource.Loading -> {
                    showProgress()
    ...
    private fun addProductToDb() {
        try {
            ...
            omniScannedItem?.skuMasterTypesList?.get(0)?.productId =
                strParentId
            matchWithViewModel.addOmniProductToPrefs(
    ...
    fun showDeleteProductDialog() {
          val  alertDialog = AlertDialog.Builder(mainActivity!!)
            ...
            alertDialog?.setMessage("The product has been added to bag successfully")
